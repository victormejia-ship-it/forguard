...
| `test_resultados_exportar.js` | Exportar Resultados a Excel y PDF (25-sep-2026, pedido de Victor: "necesito que podamos exportar en 3 aspectos excel o pdf en los formatos de forguard" — confirmado: Resumen general + Cumplimiento de pólizas + Detalle por pilar, ambos formatos para todo). Rehecho el mismo día (Victor, tras ver la primera versión de 6 hojas: "necesito que el excel siga el formato del framework proforma que te comparti, los mismos colores y cálculos" + "revisa el pdf porque se empalman las tablas"): el Excel ahora es UNA sola hoja "Framework Proforma" calcada de su archivo real (Framework_Proforma_Forguard_V2.xlsx) — los 4 pilares apilados uno debajo del otro (no una hoja por pilar), con RESUMEN FORGUARD y GASTOS DE ESTRUCTURA FORGUARD (en $0, sin fuente conectada todavía) al final, los mismos colores (navy #174A7E / slate #263746 / azul claro #E7EEF5) y las MISMAS fórmulas vivas que su Excel (`=SUM(B6:B8)`, `=B9-B16`, `=IFERROR(B17/B9,0)`, `=B9+B26+B43+B60`…) en vez de números ya calculados — probado llamando directo a `construirHojaFrameworkProforma()` e inspeccionando el XML crudo/las fórmulas exactas, con los mismos números de fila que su archivo. Cumplimiento de pólizas (que no existe en el Excel de Victor, es aporte de esta app) se queda como segunda hoja, deduplicando "Cliente / Sitio" cuando el sitio ya trae el nombre del cliente adentro (mismo criterio que armarPayloadPoliza — antes salía "NGK — NGK"). El PDF (docs/plantilla_resultados_forguard.html, SIEMPRE horizontal) reparte Cumplimiento de pólizas por CONTEO FIJO de filas en su primera versión — con datos reales (más de 20 pólizas activas) esa cuenta se quedaba corta y el pie (posición absoluta) quedaba ENCIMADO con las últimas filas ("se empalman las tablas") — así que ahora se MIDE de verdad, igual que plantilla_orden_compra.html: se prueba con 1 sola póliza (cabe en una hoja) y con 46 (fuerza varias hojas), confirmando que TODAS las filas se imprimen, que Cumplimiento sí se reparte en más de una página, y que en cada una la última fila termina antes de donde empieza el pie (sin traslape). Los dos formatos respetan puedeVerNomina() sin un candado aparte: como Analyst, "Nómina directa del pilar" sigue en el Excel pero en $0 — porque reusan literal los mismos PILARES_PROFORMA/resumenGeneralProforma/polizasActivasParaCumplimiento de la pantalla |
