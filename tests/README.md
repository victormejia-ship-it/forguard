...
| `test_resultados_proforma.js` | Segundo intento de Resultados (24-sep-2026, rehecho desde cero calcado del Excel "Framework Proforma Forguard V2"): que la app arranque ahí, el gating por rol, que sean EXACTAMENTE 4 secciones (una por pilar) con los renglones y el orden exactos del Excel, que un renglón sin conectar siga en $0; el primer renglón ya wireado (Pólizas de mantenimiento/…incluidas en precio/Refacciones y materiales): cargo cliente vs. interno, que ambos generen ingreso y costo, que el ingreso cuente por calendario de facturación (no por "cobrado"), el costo repartido parejo entre los 12 meses, y que solo cuenten las pólizas Activas; el segundo renglón (Trabajos correctivos / preventivo / Otros costos directos, de Cotizaciones): solo estatus "Realizado" cuenta (una "aprobada" no se cuela, aunque ESTATUS_COTIZACION_INGRESO sí la marque como ganada), cargoA comedor también cuenta, la fecha usa fechaRealizado con `fecha` como respaldo si falta, y un año distinto no se cuela; y el rediseño visual (25-sep-2026, pedido de Victor: gráfico a la izquierda + datos a la derecha, y un resumen general arriba): `.wrap` se ensancha SOLO en Resultados, cada bloque (los 4 pilares + el nuevo "Resumen general Forguard") trae su gráfico primero y la tabla/ranking después, el Resumen general suma los 4 pilares en vivo (KPIs + gráfico agregado + ranking de ingresos por pilar), y el tooltip real (mouse sobre un mes) muestra el detalle correcto reutilizando el mecanismo de grafica-hit/graficaTT que había quedado huérfano desde el primer intento de Resultados; y (25-sep-2026, mismo día) tablas compactas + cumplimiento de pólizas: por default cada tabla muestra solo el mes actual y los 3 anteriores (colspan de las filas de sección cuadra con las columnas visibles, el Total sigue sumando los 12 meses completos), "Ver los 12 meses"/"Últimos meses" expande o comprime las 4 tablas a la vez con un clic (estado.resultadosMeses, mismo mecanismo genérico data-accion="vista-lista" del resto de la app); y "Cumplimiento de pólizas" (gráfico de dispersión, no de barras) apilado DEBAJO del gráfico de Ingresos/Costos/Utilidad de Servicios Técnicos —no en su propia sección ancha, pedido de Victor tras ver la primera versión—, exclusivo de ese pilar (los otros 3 no lo traen): solo pólizas Activas, cumplimientoPct/hechosDebidos/serviciosDebidos calcados de calcularPoliza() sin inventar un cálculo nuevo, el color de cada punto es el mismo semáforo ok/alerta/error/neutro de siempre, y su tooltip muestra folio/estado/cumplimiento/ingreso anual; y "Nómina directa del pilar" (25-sep-2026, en Servicios Técnicos/Operativos/Proyectos — Tecnología y Control no la trae, calcado del Excel): se deriva sola del Organigrama (datos.personal, campo `pilarId`) y de una colección aparte y sensible (datos.nominaPersonal, campo `sueldoMensual`) — solo personal Activo cuenta, de baja o sin pilar asignado no se cuela, parejo en los 12 meses, y sin sueldo capturado da $0 sin tronar; y que el dato es sensible de verdad: con exactamente los mismos datos cargados, Owner ve la suma real pero Analyst ve $0 en el mismo renglón (puedeVerNomina(), más la regla de Firestore de nominaPersonal restringida a esAdmin() — no solo escondido en la pantalla); y "Proveedores / servicios subcontratados" (25-sep-2026, Servicios Técnicos): dos fuentes — gastosProveedor con estatus Por pagar/Pagado y campo `pilarId` (Cotización/Cancelado no cuentan), MÁS Órdenes de Compra Confirmada/Enviada/Recibida (Borrador/Cancelada no) que no tienen campo Pilar propio así que se suman TODAS a Servicios Técnicos (confirmado con Victor tras encontrar, al revisar el módulo, que desde el 17-sep-2026 una Orden de Compra ya es la cuenta por pagar real y gastosProveedor quedó como historial viejo) — ambas fuentes por la fecha del gasto/orden, sin IVA en el caso de las OC; y el dashboard interactivo (25-sep-2026, mismo día, pedido de Victor: "que el dash también pueda ser interactivo... se pueda desplegar un modal"): en un renglón ya conectado (`concepto.detalle`, ver PILARES_PROFORMA), clic en el nombre abre el detalle del AÑO completo y clic en un mes (o el Total) el detalle de SOLO ese mes — la lista real de pólizas/cotizaciones/personas/gastos/OC detrás del número, cada una con enlace directo a su propio módulo (irADetallePoliza/irADetalleCotizacion/irAEditarPersona/entrarGastoProveedor/entrarOrdenCompra); un renglón sin conectar (ej. "Limpieza") no tiene `detalle` y por lo tanto NO es clicable; Nómina respeta el mismo candado de rol dentro del propio detalle (Analyst ve la lista vacía); y de principio a fin con clics reales (no solo llamando la función): clic en la celda abre el modal con el registro y el total correctos, clic en el registro cierra el modal y navega a esa póliza en Pólizas |
| `test_levantamiento_equipo_hallazgo.js` | Marca/Modelo/Número de serie en Hallazgos + "Solo levantamiento (sin acción)" (25-sep-2026, pedido de Victor con captura de pantalla del modal "Nuevo hallazgo": "añade que se puedan ingresar marca, modelo y numero de serie del equipo" + "coloca una opción que solo sea 'levantamiento'... si no requiere una acción preventiva o correctiva se genere solo el levantamiento para que a la momento de generar una póliza también incluya el dato del equipo"): normalizarHallazgo() gana marca/modelo/serie (mismos nombres de campo que ya usa Activos) y ACCION_HALLAZGO gana el valor 'sin_accion' ("Solo levantamiento (sin acción)"), tratado igual que 'preventivo' en todo lo que mide gravedad — scoreHallazgo() nunca lo baja (score=1, incluso con prioridad urgente) y etiquetaSeveridadHallazgo() le da tono "ok", mientras que un correctivo urgente pendiente sigue bajando el score como siempre (regla intacta); los 3 campos existen en el modal, se guardan de verdad al capturar un hallazgo real (clic completo, no solo llamando la función), se ven en la tarjeta en pantalla y viajan al payload del PDF del levantamiento (`equipoDetalle`); y el cierre del ciclo que es el pedido real de fondo — el botón ya existente "Generar póliza desde este levantamiento" (`modalPoliza(null, l)`) armaba el renglón del equipo SIN pasarle marca/modelo/serie del hallazgo (normalizarPartida() ya tenía esos 3 campos desde antes, solo que nunca se los pasaban) — ahora el renglón nace con esos 3 datos ya puestos, verificado en los inputs reales de la tabla de la póliza (`tr[data-idx] input[data-campo="marca/modelo/serie"]`), sin tener que volver a preguntarlos |
| `test_resultados_exportar.js` | Exportar Resultados a Excel y PDF (25-sep-2026, pedido de Victor: "necesito que podamos exportar en 3 aspectos excel o pdf en los formatos de forguard" — confirmado: Resumen general + Cumplimiento de pólizas + Detalle por pilar, ambos formatos para todo). Rehecho el mismo día (Victor, tras ver la primera versión de 6 hojas: "necesito que el excel siga el formato del framework proforma que te comparti, los mismos colores y cálculos"): el Excel ahora es UNA sola hoja "Framework Proforma" calcada de su archivo real (Framework_Proforma_Forguard_V2.xlsx) — los 4 pilares apilados uno debajo del otro (no una hoja por pilar), con RESUMEN FORGUARD y GASTOS DE ESTRUCTURA FORGUARD (en $0, sin fuente conectada todavía) al final, los mismos colores (navy #174A7E / slate #263746 / azul claro #E7EEF5) y las MISMAS fórmulas vivas que su Excel (`=SUM(B6:B8)`, `=B9-B16`, `=IFERROR(B17/B9,0)`, `=B9+B26+B43+B60`…) en vez de números ya calculados — probado llamando directo a `construirHojaFrameworkProforma()` e inspeccionando el XML crudo/las fórmulas exactas, con los mismos números de fila que su archivo. "Cumplimiento de pólizas" se PAUSÓ en los dos formatos el mismo día, tras dos veredictos seguidos de Victor con datos reales (más de 40 pólizas activas) — primero "se empalman las tablas" (un conteo fijo de filas por hoja en el PDF se quedaba corto y el pie, posición absoluta, quedaba encimado con las últimas filas), y tras rehacerse con paginación MEDIDA de verdad (igual que plantilla_orden_compra.html) el mismo traslape "continua sobreponiendose" en un caso real más grande — así que Victor pidió "por el momento omite también el cumplimiento de las pólizas", "solo en el reporte exportado": se prueba que el libro exportado trae EXACTAMENTE 1 hoja (interceptando `armarZip` para contar los `xl/worksheets/*.xml` reales, no solo lo que arma la función), que el PDF trae exactamente 5 páginas (Resumen + 4 pilares) sin ninguna `table.cumpl`, que eso NO cambia aunque haya 46 pólizas activas, y que `hojaCumplimientoPolizasXlsx()` sigue viva y funcionando por su cuenta (solo se dejó de invocar desde el export real, para no rehacerla el día que se reactive) — la pantalla (bloqueCumplimientoPolizas, ver test_resultados_proforma.js) no se tocó. Los dos formatos respetan puedeVerNomina() sin un candado aparte: como Analyst, "Nómina directa del pilar" sigue en el Excel pero en $0 — porque reusan literal los mismos PILARES_PROFORMA/resumenGeneralProforma de la pantalla |
