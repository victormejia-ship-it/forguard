...
| `test_nomina.js` | Módulo nuevo "Nómina" (fase 1 del rediseño de Resultados): alta/edición/borrado, el campo Pilar, los 3 indicadores, y que sea invisible e inaccesible (`exigirAdmin()`) para cualquier rol que no sea Owner o Admin |
| `test_gastos_generales.js` | Módulo nuevo "Gastos generales" (fase 2 del rediseño de Resultados: Gasolinas, Trampas de grasa, Extracciones, Renders y demás gastos administrativos/corporativos): alta/edición/borrado, el campo Categoría (administrativo/corporativo/otro), los 3 indicadores, y que sea invisible e inaccesible (`exigirAdmin()`) para cualquier rol que no sea Owner o Admin |
| `test_resultados_pilares.js` | La vista "Por pilares" de Resultados (Framework de Proforma de Victor): `calcularResultadosPorPilar()` mete cada ingreso/costo en el pilar correcto, suma la nómina por pilar respetando el periodo, que el precio de venta de Imagen/Aperturas cuente como ingreso de Proyectos e Infraestructura solo desde "Aprobado" en adelante (agrupado por tipo: Aperturas/Remodelaciones/Otros), que los Gastos generales llenen los 3 renglones de "Gastos de estructura" agrupados por categoría, y que la vista pinte los 4 pilares + Resumen + Gastos de estructura + utilidad operativa |
| `test_servicios_operativos.js` | Módulo nuevo "Servicios Operativos" (segundo pilar del Framework de Proforma): alta/edición/borrado de un evento (Limpieza) y de un contrato de un solo renglón (Fumigación/Transporte), el toggle de cobro sin modal, el gating por rol (Owner/Admin/Analyst/Viewer sí, Técnico/Reportero/control_activos no), y que `calcularResultadosPorPilar()` ya jale ingreso/costo reales de aquí para el pilar "Servicios Operativos" |
