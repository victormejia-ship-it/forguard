...
| `test_resultados_exportar.js` | Exportar Resultados a Excel y PDF (25-sep-2026, pedido de Victor: "necesito que podamos exportar en 3 aspectos excel o pdf en los formatos de forguard" — confirmado: Resumen general + Cumplimiento de pólizas + Detalle por pilar, ambos formatos para todo): los botones "Descargar Excel"/"Generar PDF" existen en el titulo-zona de Resultados; el Excel (armado a mano con el mismo motor XLSX de descargarExcel — armarZip/filaXml/cT-cN-cX/XLSX_ESTILOS, sin ninguna librería externa) trae 6 hojas (Resumen general, Cumplimiento de pólizas y una por cada uno de los 4 pilares), probadas llamando directo a cada `hojaXxxXlsx()` e inspeccionando el XML crudo; el botón real produce un blob con el MIME de xlsx y contenido no vacío; el PDF (nueva plantilla docs/plantilla_resultados_forguard.html, copiada de plantilla_orden_compra.html con la misma identidad Forguard, SIEMPRE horizontal porque la tabla de 14 columnas no cabe en vertical) se navega de verdad (mismo criterio que test_poliza_imprimir_calendario.js): sin pantalla de "Sin datos", exactamente 6 hojas con 1 sola póliza activa (Cumplimiento se reparte por CONTEO fijo de filas, no por altura medida, a diferencia de la paginación de Cotizaciones/Órdenes de Compra/Pólizas), las 6 en horizontal, con el folio real de la póliza y el nombre de cada pilar; y que los dos formatos respeten puedeVerNomina() sin un candado aparte: como Analyst, la hoja del pilar SIGUE trayendo el renglón "Nómina directa del pilar" pero ya no con el sueldo real, sino en $0 — porque reusan literal los mismos PILARES_PROFORMA/resumenGeneralProforma/polizasActivasParaCumplimiento de la pantalla |
